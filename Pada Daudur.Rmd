...
# Change all NA values in SANS and SAPS to 0
data$SANS[is.na(data$SANS)] <- 0
...
# Summarise data
data %>% split(list(data$diagnosis, data$study)) %>%
  map(summary)
...
# Make variable for proportion of time spoken and pause duration
data$pots <- data$phonationtime/data$dur
data$paudur <- (data$dur - data$phonationtime)/data$npause
data$paudur(is.infinite(data$paudur)) <- 0

data$sd_scaled <- scale(data$sd, center = TRUE, scale = TRUE)
data$pots_scaled <- scale(data$pots, center = TRUE, scale = TRUE)
data$speechrate_scaled <- scale(data$speechrate, center = TRUE, scale = TRUE)
data$paudur_scaled <- scale(data$paudur, center = TRUE, scale = TRUE)


m <- lmer(sd ~ diagnosis + (1|subject), data, REML = FALSE)
m2 <- lmer(pots ~ diagnosis + (1|subject), data, REML = FALSE)
m3 <- lmer(speechrate ~ diagnosis + (1subject|), data, REML = FALSE)
m4 <- lmer(paudur ~ diagnosis + (1|subject), data, REML = FALSE)
