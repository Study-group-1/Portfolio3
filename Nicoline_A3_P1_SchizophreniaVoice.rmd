...
pacman::p_load(tidyverse, tidymodels, stringr, readr, dplyr, stringr, lmerTest, Metrics, caret)
```
...
Look at the results reported in the paper (see meta-analysis in the readings) and see whether they are similar to those you get.
    ...
    which fixed and random effects should be included, given your dataset? E.g. what about language and study, age and gender?
    Discuss also how studies and languages should play a role in your analyses. E.g. should you analyze each study individually? Or
    each language individually? Or all together? Each of these choices makes some assumptions about how similar you expect the
    studies/languages to be.
...
1) Discuss whether you should run the analysis on all studies and both languages at the same time
...
```{r}
data$SANS[is.na(data$SANS)] <- 0 #we prob won't use SANS + SAPS
data$SAPS[is.na(data$SAPS)] <- 0
data <- na.omit(data)
...
data %>% 
    ...
    summarize(N = n(), 
              ...
              sd_SAPS = sd(SAPS, na.rm = T),
              mean_VerbalIQ = mean(VerbalIQ, na.rm = T), 
              mean_NonVerbalIQ = mean(NonVerbalIQ, na.rm = T), 
              mean_TotalIQ = mean(TotalIQ, na.rm = T),
              mean_PitchMean = mean(mean, na.rm = T),
              mean_PitchSD = mean(sd, na.rm = T),
              mean_PitchIQR = mean(iqr, na.rm = T))
...
str(data)
data %>% 
    split(list(data$study, data$diagnosis)) %>% 
                   map(summary)

#diagnosis schitzofrenia = 1, control = 0

data %>% split(data$study) %>%
  map(summary)
...
```{r}
# Compare results regarding diagnosis - do we see anything significant? 
...
# proportion (pots) = phonationtime/dur
data$pots <- data$phonationtime/data$duration
data$pause_dur <- (data$duration - data$phonationtime) / data$npause

# pause duration (pause_dur) =  (dur - phonationtime) / number of pauses
data$pause_dur[is.infinite(data$pause_dur)] <- 0 #when we divide by 0 we get "inf" values. The time of a pause should however be 0 when there ar eno pauses. 

#scaling
#We need to not scale all the data together, as it will remove the particularities of the languages, or the study for that matter. Scaling grouped by language::
data <- data %>% 
    group_by(Language) %>% 
    mutate(sd_scaled = scale(sd), 
           pots_scaled = scale(pots),
           pause_dur_scaled = scale(pause_dur),
           speechrate_scaled = scale(speechrate), 
           iqr_scaled = scale(iqr),
           mean_scaled = scale(mean))

levels(data$Language)
data$study <- as.factor(data$study)
levels(data$study)

#add random effects? random slope for study or language? (safe to assume that different languages at least has different variations in pitch +++)
m1 <- lmerTest::lmer(sd_scaled ~ diagnosis + Language + (1 |subject), data, REML = FALSE)
m2 <- lmerTest::lmer(speechrate_scaled ~ diagnosis + Language + (1|subject), data, REML = FALSE)
m3 <- lmerTest::lmer(pots_scaled ~ diagnosis + Language + (1|subject), data, REML = FALSE)
m4 <- lmerTest::lmer(pause_dur_scaled ~ diagnosis + Language + (1|subject), data, REML = FALSE)
...
```{r}
k <- 6
folds <- createFolds(unique(data$subject), k = k, list = TRUE, returnTrain = FALSE)
...
for (fold in folds){
  ...
  model = lmer(sd_scaled ~ diagnosis + Language + (1 |subject), train)
  test$prediction = predict(model, test, allow.new.levels = TRUE)
  train$prediction = fitted(model)
  trainRMSE[i] = rmse(train$sd_scaled, fitted(model))
  testRMSE[i] = rmse(test$sd_scaled, test$prediction)
  i = i + 1
}
...
RMSE <- data.frame(trainRMSE, testRMSE)
...
cross_validate <- function(x, y) {
  ...
  rm <- data.frame("model" = y, "train" = trainRMSE, "test" = testRMSE, "meantrain" = rep(mean(trainRMSE), 6), "meantest" = rep(mean(testRMSE), 6), "meandiff" = rep((mean(testRMSE) - mean(trainRMSE)), 6), "sdtrain" = rep(sd(trainRMSE), 6), "sdtest" = rep(sd(testRMSE), 6))
  return(rm)
}
