...
data <- data %>% filter(Language == 'Danish')


# Now we save them
write.csv(data, "Scizophrenia_data_merged.csv")
```
...
# Gustav
str(data)
data %>% split(list(data$study, data$diagnosis)) %>% 
                   map(summary)
...
#make models
m1 <- lmerTest::lmer(sd_scaled ~ diagnosis + (1|subject),
                     data, 
                     REML = FALSE,
                     control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
m3 <- lmerTest::lmer(pots_scaled ~ diagnosis + (1|subject),
                     data,
                     REML = FALSE,
                     control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
m2 <- lmerTest::lmer(speechrate_scaled ~ diagnosis + (1|subject),
                     data,
                     REML = FALSE,
                     control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
m4 <- lmerTest::lmer(pause_dur_scaled ~ diagnosis + (1|subject),
                     data,
                     REML = FALSE,
                     control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
